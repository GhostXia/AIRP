...
`.github/workflows/pr-gate.yml` automatically enforces the Rust workspace, UI/WebUI and existing production topology regression gates. `.github/workflows/webui-windows-build.yml` handles the current portable Windows WebUI artifact and real-Chrome acceptance; `.github/workflows/manual-build.yml` only retains the long-term Tauri desktop build. The audit bot is a pre-merge blocking gate: a fully-green local run only permits opening a PR; you must wait for the audit to pass and a human review to decide whether to merge.
