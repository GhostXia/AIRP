...
- P0 首方部署 artifact 与真实 topology smoke 已落地，P1 代码候选已形成但仍待真实用户验证；正式升级/自动回滚流程、发布签名及 P1/P2 产品门禁仍缺，因此尚非正式发布。SBOM/notices 已由 PR #218 `tools/dep-governance/` 提供离线生成（SPDX-2.3 / CycloneDX 1.5 / 人类可读声明，当前快照在 `docs/sbom/`），但尚未嵌入 release pipeline 作为强制度量。
...
2. P1 代码候选已形成；第一优先级是用真实 provider + 真实浏览器 + 生产拓扑执行真实用户逐阶段验收，分别记录刷新恢复与服务重启恢复，并修复实际出现的首聊阻塞、永久 loading、不可行动错误、secret 泄露、虚假成功和关键资产静默损坏；
3. data root、升级前人工冷备份和恢复旧目录/旧版本的 P1 逃生说明已交付；现在对至少 5 名符合画像的用户执行首聊完成率验证，不符合画像者单独报告、不计入分母；
