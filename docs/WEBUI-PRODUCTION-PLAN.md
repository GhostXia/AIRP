...
> 状态：当前近期执行主入口
>
> 基线日期：2026-07-23，`main@03ffaf6` / PR #297
>
...
- **当前缺口**：PR #294 已用权威样板壳重建独立 6-stage onboarding，并把完整首轮流式对话纳入 production browser smoke；PR #294 的 mock-provider production topology、页面刷新、服务重启和 Windows 真 Chrome 候选验证已通过。PR #297 已扩展角色/会话删除、FTS5 搜索、Persona 删除/解绑和 state history/schema 操作入口。尚未完成的是通用发布验收：用真实 provider 在目标浏览器/设备复核首聊、刷新与重启恢复，并根据真实用户首聊补实际阻塞；不应把这项缺口写成 PR #294/#297 的 CI 未验证。旧 PR #212 的 Port/dynamic-import 宿主实现已被替换；Shadow DOM 与 Port 版本协商只适用于已淘汰宿主边界，合并后应重新评估 #210/#211，而不是机械移植。
...
- Engine 的 durable message ID 与 cursor history 合同仍在；PR #294 WebUI 当前使用 200 条有界读取、全量 DOM 重建与 rollback-by-ID，cursor"加载更早"和增量 DOM 尚未迁入新壳。
...
7. 对 branch/edit、完整 Persona/Preset 生命周期、大对象上传和高级 developer controls 做诚实取舍：未交付能力隐藏、禁用或明确标为非 P1，不为追求表面完整阻塞首聊。
...
> 2026-07-23 PR #297 合并后校准：开发优先于修补，用户反馈优先于审计遗留项。
...
5. WebUI production umbrella issue 为 [#130](https://github.com/GhostXia/AIRP/issues/130)；P0-P3 在其中按独立验收切片追踪；
