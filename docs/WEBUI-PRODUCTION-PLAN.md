...
- 刷新页面或重启服务后可以继续，常见网络/认证/provider 失败可重试；
...
市场验证不得由开发者自测或自动化 smoke 代替。每名自愿试用者使用同一份记录：匿名编号、目标用户画像是否匹配、候选版本、浏览器/设备、开始与结束时间、完成到哪个阶段、是否在不读开发文档/不打开开发工具的情况下完成首聊、刷新后是否继续、重启后是否继续、失败的阶段×类型、是否重试成功、是否愿意再次使用、经同意保留的原话摘要。达到至少 5 人后，以“成功完成首聊人数 ÷ 总参与人数”计算完成率；低于 80% 或存在 secret/资产损坏事件时 P1 不退出。当前状态必须写“待真实用户验证”，不得用 mock、agent 或维护者代填。
