# 对话体验升级：SillyTavern 校准版

> **计划建立时间**：2026-07-19
>
> **审计/规划模型**：Qwen3.8-Max-Preview
>
> **校准来源**：SillyTavern (github.com/SillyTavern/SillyTavern) 公开文档与发布记录
>
...
本方案以 SillyTavern（github.com/SillyTavern/SillyTavern）的真实 RP 用户日常工作流为校准基准。ST 是 RP 社区事实标准客户端（300+ 贡献者、3 年独立开发），其高频交互模式代表了 RP 用户的真实需求。
...
- `RegenRequest` 仅增加 `user_id: Option<String>`（用于 per-user 数据隔离）。
  ...
  （ST 校准：单用户本地场景，regen 用当前活跃配置即可，无需请求级覆盖）。
- handler 改为：
...
- `ChatService::delete_message(character_id, session_id, message_id)` — 删除指定单条消息，保留其余消息顺序不变。（ST 校准：ST 删除是移除单条，不是 rollback 删除后续所有。）
...
- 移除旧的"删除后手动重发"逻辑和 confirm 弹窗（ST 校准：ST regen 无确认弹窗，直接替换）
...
- 空消息发送 = 继续（ST 校准：ST 用户习惯发空消息触发 continue）
...
- **保存**：调用 engine 更新该消息文本（需 engine 支持 `PUT /v1/chat/message` 或等效），不触发重新生成
- **保存并重新生成**（可选按钮）：保存后调用 rollback 到该消息之前 + regen SSE
- 取消编辑 → 恢复原文
- （ST 校准：ST 的 "Auto-save Message Edits" 和 "Save and re-edit" 是分离操作，编辑不自动触发 regen）
...
- （ST 校准：ST 删除是移除单条消息，不是 rollback 删除后续所有）
...
## 新增 Issue（SillyTavern 校准发现的真实需求，当前 PR 不含但需跟踪）

以下功能经 ST 校准确认为 RP 用户真实日常需求，但范围超出本次 PR，应创建 GitHub issue 跟踪：
