...
**Stage 4 文件类型**：
- PNG 角色卡：检测 magic bytes，base64 编码，POST `{ card_png_base64 }`。
- character_book JSON / preset JSON：先解析验证，按类型路由到 `/v1/characters/import`（`{ card_json }`）或 `/v1/presets/import`（`{ preset_id, preset_json }`）。
...
function shouldShowOnboarding() {
  ...
  // 脱同步信号：localStorage 里的 character_id 失效（被外部删除）
...
async function bootstrapApp() {
  if (shouldShowOnboarding()) {
    try {
      ...
      mountOnboarding(container, hostPort);
...
**skip 也写 `airp_onboarded=true`**：避免每次加载打扰已配置用户；脱同步检测仍保护（`airp_character_id` 失效时重新触发）。这是 C 方案的核心——标志是"别再烦我"的偏好，不是"配置完整"的断言。
...
向导 `mountOnboarding` 包一层顶层 try/catch，捕获渲染与状态机之外的未预期错误：

```js
export function mountOnboarding(container, hostPort) {
  if (hostPort.version !== 1) throw new Error('onboarding: hostPort.version must be 1');  // F3

  let stage = 1;
  let state = {};
  const listeners = [];   // 注册的事件监听，cleanup 时遍历移除

  try {
    renderStage(container, stage, hostPort, state);                // 主渲染逻辑
  } catch (err) {
    renderCrashFallback(container, err, () => {
      container.innerHTML = '';
      stage = 1; state = {};
      renderStage(container, stage, hostPort, state);              // [重试向导]
    }, () => {
      hostPort.onSkip();                                           // [退回手动配置]——走宿主 onSkip
    });
...
**关键约束**：F4 的"退回手动配置"通过 `hostPort.onSkip()` 实现——宿主 `onSkip` 会写 `airp_onboarded=true` + `scheduleAutoConnect()`。这意味着用户从崩溃退出后**不会下次加载又被向导拦截**（除非脱同步触发）。这是有意的：崩溃的向导不应无限循环拦截用户。
