...
这些远端证据覆盖 workspace fmt/build/严格 Clippy/tests、warning-free rustdoc、神圣提示词不变式、Node 20.19 下的 UI typecheck 与 Vitest、WebUI 脚本/纯函数测试，以及真实 HTTPS/Chrome production topology；Rust workspace 还覆盖 scene worldbook 逐条 provenance、精确重复项首来源和 prompt 字节顺序回归。该结果证明 `c38e7ec` 与 PR #196 head；后续变更不得沿用为新结果。
