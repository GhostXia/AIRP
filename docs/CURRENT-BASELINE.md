...
> 基线日期：2026-07-19
>
> 前置实现基线：`main@db20b14` / PR #236；本文件所在提交增加 Windows 便携 WebUI 候选
>
...
- WebUI 的 50 条窗口不是虚拟列表；Tauri/Vue 的 10k/100k 性能与内存上界仍属长期路线。Windows 便携 WebUI 已有本机 artifact/Chrome 自动 smoke，但尚需 GitHub artifact 门禁与真实 provider 人工验收。
- `card_path` 只适用于可信本地桌面调用；远端/Web 调用必须使用受控 content upload，不能把服务端任意路径读取暴露给不可信调用方。

## 4. 当前执行顺序

**P1 有限试用代码候选已形成；当前近期目标是以 Windows 便携 WebUI 尽快落地，Tauri 长期开发，Docker/WSL 不作为当前依赖。**按以下顺序执行：

1. **先交付 Windows 便携 WebUI artifact**：`airp-core.exe` 在 loopback 同源托管 WebUI/API，用户解压双击运行，不依赖 Node、Cargo、Docker、WSL 或 Tauri；`data/` 与 `config.json` 固定在包内，provider key 集中保存在默认不回显的 `data/secrets.json`，升级前备份并迁移 `data/`。
2. **建立可重复验收**：Windows artifact + 真实 Chrome 自动 smoke 先通过，再用真实 provider 贯通 onboarding → 首聊 → 页面刷新后继续 → engine 重启后继续并独立记录；保留失败阶段×类型和重试结果。
3. **修复阻塞并继续完善**：任何首聊阻断、永久 loading、不可行动错误、secret 泄露、虚假成功或关键资产静默损坏立即修复并重新验证；在黄金路径稳定前继续实现直接影响可用性的功能，不把尚未具备条件的外部试用招募设为开发门禁。
4. **保持 P1 取舍**：人工备份/回滚逃生路径保留，但不把它写成自动恢复能力；不新增 asset revision 类型、revision chip、provenance 展示或 dependency-governance 自动化。Persona/Preset 高级生命周期、HTTP/UI 受控 dry-run、完整 revision/collision/overwrite/provenance 审计及 #220 性能/重构项后移到 P2，除非源码审计证明其直接触及上述底线。
5. **P1 工程门禁通过后再进入 P2/P3**：P2 选择一种高价值资产完成“工作副本/读取真值 → revision 完整性校验 → 差异比较 → 回滚 → 恢复 → 导出”的端到端 vertical slice；P3 再执行浏览器兼容、安全负向、旧数据升级、备份恢复、长会话 soak、artifact、签名与回滚演练。

本文件所在提交的本机 Windows 证据：严格 Clippy 与 warning-free rustdoc 通过；workspace 741 passed / 1 ignored、25 integration、protocol 6、现有 Tauri 9、Vitest 98、WebUI 98、dependency-governance 90、神圣提示词不变式均通过；release ZIP 从打包目录启动并通过 HTTP 安全边界与真实 Chrome smoke。该证据仍须由 ready PR 的远端门禁与审计 bot 复核。

开放 issue 还包括审计/流程/文档遗留项；不要在本文复制完整 issue 列表。开始工作前使用 `gh issue list --state open` 获取实时状态。

## 5. 最近验证证据

PR #232 最终 head `29b52fa` 在 [PR gate run 29645599733](https://github.com/GhostXia/AIRP/actions/runs/29645599733) 中通过全部代码门禁；其代码树随后以 merge commit `main@2a14b7e` 合入：

- `Rust workspace`：通过；
- `UI and WebUI`：通过；
- `Production topology`：通过；
- `CodeRabbit`：最终复审通过，最近一轮未产生可操作意见。

远端 run 的 `cargo test --workspace --locked` = **756 lib tests（740 engine pass + 1 ignored + 6 protocol + 9 ui）+ 25 integration tests**（4 agent_run + 11 openai_compat + 5 production_startup + 5 sse_wiremock）；`node --test webui/tests/*.test.mjs` = **97 tests**；`ui/` Vitest = **98 tests across 13 files**；production HTTPS topology = **104 checks / 0 failures**。本次校准在 `main@2a14b7e` 上本地复算 `node --test tools/dep-governance/tests/*.test.mjs` = **90 tests**。

这些证据覆盖 workspace fmt/build/严格 Clippy/tests、warning-free rustdoc、干净提示词不变式 `subagent_context_has_no_orchestrator_noise`、Node 20.19 workflow step runtime（`setup-node@v6` action wrapper 自身跑在 Node 24）下的 UI typecheck 与 Vitest、97 项 WebUI 回归、依赖发现/路由/SBOM 测试，以及真实 HTTPS/Chrome production topology smoke。新增回归覆盖 timeline 在 user message 落盘后才推进、关键 finalization 写入失败不发 `done`、结构化 SSE commit state、ambiguous/partial commit 不盲重发、Persona 路径穿越与清理失败关闭、quoted JSON secret 脱敏、`PathEscape` 公开消息脱敏、session 清理和生产镜像 `shared.js` 装载。该结果证明 PR #232 最终 head 及其合入 `main@2a14b7e` 的代码树；后续变更不得沿用为新结果。

本基线吸收的最近合并序列（`63f1c5b → 2a14b7e`）：PR #229（post-#227 基线校准、测试计数纠错、SPDX 表达式 AST/语法修复与审计收口）→ PR #232（P1 首聊失败关闭、结构化 SSE 重试边界、Persona 删除安全、secret/error 脱敏、人工备份/回滚逃生路径及 P1 验收口径收口）。
