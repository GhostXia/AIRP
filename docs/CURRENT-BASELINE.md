...
- WebUI 已按权威控制台样板重建为无构建、多页面、CSP 兼容的同源客户端（PR #294 候选）：角色导入/搜索、Provider 设置与模型验证、Preset/Persona 基础管理、命名 session、流式聊天、continue/regen、durable-ID rollback/edit/delete、Swipe/branch 管理、Agent Run、装配预览和诊断均直接连接 Engine。聊天页当前一次读取最多 200 条并重建 DOM；旧版的 cursor“加载更早”、增量 DOM 复用和聊天主面板装配 chips 尚未迁入新壳，不得继续写作现有 UI 能力。Engine 侧 `PromptAssemblyTrace`、统一有效配置摘要和 revision 诊断合同仍保留，用户可在装配预览/相关管理页读取其中一部分。
- 首次启动已在新样板壳中恢复独立 6-stage onboarding（PR #294 候选）：部署检查 → Provider 配置 → 模型验证 → 角色导入 → Persona 用户资料/Preset 选择 → 真实首轮流式对话。根入口按浏览器本地完成标记进入向导或角色页；密钥不进入 URL/浏览器存储，SSE 未收到 `done` 或传输中断一律标为 commit state unknown 并要求刷新历史确认。旧 PR #212 的 Port/dynamic-import `webui/app.js` 实现已被替换，不再是当前运行时边界。
