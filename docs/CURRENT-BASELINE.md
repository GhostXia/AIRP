...
> 基线日期：2026-07-24（`main@bc72c48`；PR #300 + PR #305 已合并）
>
> 已合并实现基线：`main@bc72c48`；PR #300（Agent 驱动浏览器探索测试层 MVP）与 PR #305（persona bindings DELETE 修复）均已合并并通过全部 CI 门禁
>
...
- WebUI 已按权威控制台样板重建为无构建、多页面、CSP 兼容的同源客户端（PR #294 已合并）：角色导入/搜索、Provider 设置与模型验证、Preset/Persona 基础管理、命名 session、流式聊天、continue/regen、durable-ID rollback/edit/delete、Swipe/branch 管理、Agent Run、装配预览和诊断均直接连接 Engine。聊天页当前一次读取最多 200 条并重建 DOM；旧版的 cursor-based "加载更早"、增量 DOM 复用和聊天主面板装配 chips 尚未迁入新壳，不得继续写作现有 UI 能力。Engine 侧 `PromptAssemblyTrace`、统一有效配置摘要和 revision 诊断合同仍保留，用户可在装配预览/相关管理页读取其中一部分。
- 首次启动已在新样板壳中恢复独立 6-stage onboarding（PR #294 已合并）：部署检查 → Provider 配置 → 模型验证 → 角色导入 → Persona 用户资料/Preset 选择 → 真实首轮流式对话。根入口按浏览器本地完成标记进入向导或角色页；密钥不进入 URL/浏览器存储，SSE 未收到 `done` 或传输中断一律标为 commit state unknown 并要求刷新历史确认。旧 PR #212 的 Port/dynamic-import `webui/app.js` 实现已被替换，不再是当前运行时边界。
...
- WebUI 的 200 条读取窗口不是 cursor 分页或虚拟列表；Tauri/Vue 的 10k/100k 性能与内存上界仍属长期路线。Windows 便携 WebUI 已有本机 artifact/Chrome 自动 smoke，并由 GitHub artifact 门禁覆盖；真实 provider 人工验收仍需按候选版本记录。Swipe、branch 切换和用户消息 edit 已交付，但新样板壳的完整长历史浏览与主聊天装配诊断尚未迁回。
- `card_path` 只适用于可信本地桌面调用；远端/Web 调用必须使用受控 content upload，不能把服务端任意路径读取暴露给不可信调用方。

# 4. 当前执行顺序

> 2026-07-24 PR #305 合并后校准：Agent 浏览器探索测试层 MVP（PR #300）、persona bindings DELETE 修复（PR #305）和独立审计归档（PR #306）均已合并。项目最大风险仍是工程治理远超产品成熟度、首聊迟迟未有真实用户。以下顺序体现"先让产品活过来，再让它健康"的原则。
...
PR #305 squash-merge `bc72c48` 通过全部代码门禁（Rust workspace / UI and WebUI / Production topology / Portable Windows WebUI / CodeRabbit）。

PR #300 新增 `tools/agent-exploration/`（runner + classifier + reporter + 4 任务 + CI workflow）和 `webui/assets/agent-test-harness.js`（6 页面注入），`webui/tests/agent-harness.test.mjs` 覆盖 harness 合同。PR #305 更新 `webui/tests/operations.test.mjs` 覆盖 persona bindings query string 修复。

本基线吸收的最近合并序列（`03ffaf6 → bc72c48`）：PR #298（docs 校准 + Agent 浏览器探索计划）→ PR #300（Agent 驱动浏览器探索测试层 MVP + 审计 B1-B9 修复）→ PR #306（WorkBuddy 独立审计归档）→ PR #305（persona bindings DELETE query string 修复）。
