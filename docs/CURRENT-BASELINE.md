...
- `PromptAssemblyTrace` 已接入真实 single/scene chat 装配路径，按 provider payload 顺序记录 card/lorebook/state/preset/scene/memory/history/user 的显式 provenance；`POST /v1/chat/preview` 复用该路径但不推进时间线、不创建会话或修复 metadata，且不返回 prompt 正文、API key 或 endpoint 明文。
