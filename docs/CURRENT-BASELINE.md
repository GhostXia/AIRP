...
1. **先交付 Windows 便携 WebUI artifact**：`airp-core.exe` 在 loopback 同源托管 WebUI/API，用户解压双击运行，不依赖 Node、Cargo、Docker、WSL 或 Tauri；`data/` 与 `config.json` 固定在包内，升级前备份并迁移 `data/`。
