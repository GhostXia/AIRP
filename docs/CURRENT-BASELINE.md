...
- 当前近期目标是 **P1 有限试用版**，不是“正式发布”：P1 只承诺少量自愿用户可完成首聊、刷新后继续、常见失败可重试，且 secret 不泄露、关键资产不被静默损坏。完整 RP 资产生命周期、base lock / drift / rollback、版本化 migration、自动备份/恢复、可恢复删除、升级回滚、运维 runbook、浏览器矩阵与长会话 soak 仍属于 P2/P3 或正式发布门禁。
...
当前唯一近期目标是尽快形成 **P1 有限试用候选版**。按以下顺序执行：

1. **闭合首聊黄金路径**：真实用户通过 onboarding 完成部署健康检查 → provider 配置 → 模型验证 → 角色导入 → Persona/Preset 选择 → 首轮流式对话；随后刷新页面或重启服务仍能继续当前会话。优先修复任何阻断该路径、让用户必须读文档或打开开发工作台的问题。
2. **守住不可延期底线**：provider/API key 不进入 URL、WebUI 持久存储、日志或错误详情；认证、网络、超时、SSE、磁盘与持久化失败必须 fail-visible 并提供重试/处理动作；角色卡、世界书、Persona、Preset、session 与聊天历史不得静默删除、覆盖或在失败后返回虚假成功。
3. **提供有限试用运维兜底**：明确 data root、升级前人工备份和恢复旧目录/旧版本的方法；错误信息能定位到可行动步骤。P1 不要求自动 backup/restore、migration registry、soft-delete 或完整 runbook。
4. **冻结非主链扩张**：不新增 asset revision 类型、revision chip、provenance 展示或 dependency-governance 自动化；现有 revision 只作为实验性基础设施，不向 P1 用户承诺可恢复。Persona/Preset 高级生命周期、完整 Preset 生命周期、HTTP/UI 受控 dry-run、完整 revision/collision/overwrite/provenance 审计及 #220 性能/重构项后移到 P2，除非源码审计证明其直接导致 secret 泄露、虚假成功或用户资产损坏。
5. **完成 P1 试用验证后再扩展**：至少 5 名符合画像的自愿用户中首聊完成率 ≥80%，记录失败阶段与类型；未达到时继续修黄金路径，不启动新的横向基础设施。P1 候选版形成后统一校准基线一次，不再为每个小 PR 单独创建 baseline recalibration。
6. P2 再选择一种高价值资产完成“工作副本/读取真值 → revision 完整性校验 → 差异比较 → 回滚 → 恢复 → 导出”的端到端 vertical slice，然后决定是否继续横向扩展其余 asset revision；P3 再执行浏览器兼容、安全负向、旧数据升级、备份恢复、长会话 soak、artifact、签名与回滚演练。
