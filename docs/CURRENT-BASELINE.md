...
> 基线日期：2026-07-24（`main@44b9c83`；PR #301 + PR #310 已合并）
>
> 已合并实现基线：`main@44b9c83`；PR #301（Linux WebUI CI + 样板/WebUI 同步 + CI 并行拆分）与 PR #310（WebUI 展现层 + 体验打磨 + Engine 加固）均已合并并通过全部 CI 门禁
>
...
- `deploy/linux-webui/`：Linux 便携 WebUI artifact（musl 静态链接，手动触发 CI）；
...
- 默认 Agent registry 有 27 个工具（含四阶段新增的 npc_action/update_relationship/advance_plot/get_plot_status/trigger_world_event/list_world_events/session_search 等）；运行时目录由 `GET /v1/agent/tools` 提供，执行受 capability、allowlist 与 destructive confirmation 约束。
...
- WebUI 已按权威控制台样板重建为无构建、多页面、CSP 兼容的同源客户端（PR #294 已合并）：角色导入/搜索、Provider 设置与模型验证、Preset/Persona 基础管理、命名 session、流式聊天、continue/regen、durable-ID rollback/edit/delete、Swipe/branch 管理、Agent Run、装配预览和诊断均直接连接 Engine。聊天页已实现 50 条/页 cursor 分页 + 滚动位置补偿 + "加载更早"增量 prepend（PR #310 #122）；Swipe 内联候选切换、Branch 可视化淡化与切换已迁入聊天主面板。Engine 侧 `PromptAssemblyTrace`、统一有效配置摘要和 revision 诊断合同仍保留，用户可在装配预览/相关管理页读取其中一部分。
- 首次启动已在新样板壳中恢复独立 6-stage onboarding（PR #294 已合并）：部署检查 → Provider 配置 → 模型验证 → 角色导入 → Persona 用户资料/Preset 选择 → 真实首轮流式对话。根入口通过 `GET /health` 的 `onboarded` 字段判断进入向导或角色页（PR #310 #303 将状态持久化到 Engine data_root，localStorage 降为离线后备）；密钥不进入 URL/浏览器存储，SSE 未收到 `done` 或传输中断一律标为 commit state unknown 并要求刷新历史确认。旧 PR #212 的 Port/dynamic-import `webui/app.js` 实现已被替换，不再是当前运行时边界。
...
- WebUI 操作入口已扩展（PR #297 已合并）：角色删除（`DELETE /v1/characters/:id`，带确认）、会话删除（`DELETE /v1/sessions/:char/:session`，带确认）、FTS5 全文历史搜索（`POST /v1/chat/search`）、Persona 删除与角色解绑（`DELETE .../personas/:id` + `DELETE .../bindings`，default Persona 有 guard）、角色状态变更历史（`GET .../state/history`）与状态 JSON Schema（`GET .../state/schema`）均已接入 WebUI。`onboarding.js` 与 `console-runtime.js` 的 field/input helper 已加 `<select>` 防御（#295 §2）。
...
- Agent 驱动浏览器探索测试层 MVP 已交付（PR #300）：`tools/agent-exploration/` 提供 runner、PR-diff classifier、reporter、PR comment 发布器和 4 个探索任务（onboarding-firstchat-refresh、regen-swipe-refresh、edit-branch-switch-refresh、memory-roundtrip）；`.github/workflows/agent-browser-exploration.yml` 在 PR 触发时自动运行；`webui/assets/agent-test-harness.js` 注入 6 个关键页面提供 DOM 快照与操作接口。无 LLM API key 时自动进入 fallback 模式（内置最小 smoke 脚本），仍可验证 topology + harness + runner + reporter 端到端 pipeline。
- WebUI persona bindings DELETE 已修复（PR #305）：`character_id` 改由 query string 传递，与 daemon handler 的 `Query` 提取器对齐。

### Linux 便携 WebUI 与 CI 并行化（PR #301）

- Linux 便携 WebUI 包已交付：`deploy/linux-webui/` 提供 musl 静态链接 build 脚本（`build.sh`）、启动器（`start-airp.sh`，含 smoke 模式与 data dir chmod 0700）和用户说明；`.github/workflows/linux-webui-build.yml` 手动触发构建并上传 tar.gz artifact。与 Windows 便携包独立，action SHA 全部 pin。
- 样板/WebUI 同步（issue #304）：model picker（保存后自动拉取 `/v1/models`）、NL 区（规划中占位）、JSON 折叠区、世界书操作列（编辑/删除/开关）、05 屏重命名为「预设」、33 屏向导模型选择。
- CI Rust job 并行拆分：原串行 ~10min 拆为 rust-lint（fmt+clippy）、rust-test（workspace tests+sacred invariants）、rust-doc（rustdoc -D warnings）三个并行 job，PR gate 墙钟降至 ~5min。

### WebUI 展现层与 Engine 加固（PR #310）

- Onboarding 状态迁移到 Engine data_root（#303）：`/health` 增加 `onboarded` 字段；新增 `POST /v1/onboarding/complete` 端点（幂等写入 `.onboarded` 标记）；`entry.js`/`onboarding.js`/`chat-space.js` 改用 Engine 端点，localStorage 降为离线后备。
- 聊天页体验核心：Swipe 内联（消息气泡内 ‹ n/m › 候选切换）；Branch 可视化（非活动分支淡化 + 切到此分支按钮）；#122 长历史窗口化（50 条/页 + cursor 分页 + 滚动位置补偿 + "加载更早"增量 prepend）。
- Engine 新能力展现：#116 Style/Drift 面板（新页面 `32-style-review.html`，Soul-Drift 编辑/保存 + 风格审查运行/结果展示）；World Events 面板（`GET /v1/characters/:id/world-events` 集成到场景页）；记忆面板增强（容量进度条 + 自动抽取/压缩说明）。
- 角色工作台增强（#304）：世界书条目编辑/删除/开关实际可用。
- 体验打磨：错误提示可行动化（`actionableHint` 按错误类型给操作建议）；会话切换顺滑（过渡动画 + opacity 渐变）；#307 DOM 快照脱敏（只取元素自身文本，不泄漏子节点）。
- Engine 加固：#281 `advance_plot`/`update_relationship` 加 dry-run 确认；#274 记忆 capacity 热重载（`AIRP_MEMORY_CAP` 环境变量）；#284 确认 `with_session` 已覆盖 per-session mutex。
...
- WebUI 的 50 条/页 cursor 分页已交付（PR #310 #122），但尚非虚拟列表；Tauri/Vue 的 10k/100k 性能与内存上界仍属长期路线。Windows 便携 WebUI 已有本机 artifact/Chrome 自动 smoke，Linux 便携包已有 musl 静态链接构建脚本与 CI artifact 门禁；真实 provider 人工验收仍需按候选版本记录。Swipe、branch 切换、用户消息 edit、风格审查/漂移编辑、世界事件展示均已交付，但聊天主面板装配诊断 chips 尚未迁回。
- `card_path` 只适用于可信本地桌面调用；远端/Web 调用必须使用受控 content upload，不能把服务端任意路径读取暴露给不可信调用方。

# 4. 当前执行顺序

> 2026-07-24 PR #310 合并后校准：Linux 便携 WebUI + CI 并行拆分（PR #301）、WebUI 展现层 + 体验打磨 + Engine 加固（PR #310）均已合并。项目最大风险仍是工程治理远超产品成熟度、首聊迟迟未有真实用户。以下顺序体现“先让产品活过来，再让它健康”的原则。
...
4. **按需加固（审计 #248 选择性修补）**：以下项在真实使用中实际触发或分发前必须处理时才修：
   ...
   - WebUI 类型安全（B-01）：新多页面 runtime 出现第二位长期维护者或任一脚本再次接近单体规模前引入；
...
PR #310 squash-merge `44b9c83` 通过全部代码门禁（Rust workspace 3 并行 job / UI and WebUI / Production topology / Portable Windows WebUI / CodeRabbit）。Engine 906 pass + WebUI 46 pass。

PR #301 新增 `deploy/linux-webui/`（build.sh + start-airp.sh + README.txt）、`.github/workflows/linux-webui-build.yml`、样板/WebUI 同步（model picker、NL 区、JSON 折叠、操作列、33 屏）、CI 并行拆分（pr-gate.yml rust-lint/rust-test/rust-doc）。审计修复 H-1/H-2/M-1~M-4/L-1~L-5 + 独立复审 NEW-1~NEW-5 均已落地。

本基线吸收的最近合并序列（`bc72c48 → 44b9c83`）：PR #301（Linux WebUI CI + 样板同步 + 审计修复 + CI 并行拆分）→ PR #309（enhancement batch，已 revert）→ PR #310（enhancement batch 重新应用）。
