...
| Conversation runtime | UI 无关的 versioned manifest、append-only event journal、policy registry、scene round-robin 回合执行 | `/v1/conversations*`、`/v1/conversation-policies`；未知策略执行 fail-closed | 尚未绑定具体 UI，客户端只消费 Engine 合同 | projection、取消/未知提交恢复、长上下文压缩与外部策略加载仍开放 |
