...
> 基线日期：2026-07-16
>
> 实现基线：`main@c8a3f1a` + `codex/115-scene-lorebook-provenance` 工作分支
>
...
- `PromptAssemblyTrace` 已接入真实 single/scene chat 装配路径，按 provider payload 顺序记录 card/lorebook/state/preset/scene/memory/history/user 的显式 provenance；scene 合并角色与场景 worldbook 后仍按命中条目保留逻辑来源和源文档条目序号，不暴露本机路径。`POST /v1/chat/preview` 复用该路径但不推进时间线、不创建会话或修复 metadata，且不返回 prompt 正文、API key 或 endpoint 明文。
...
- WebUI 已有 provider 配置、角色导入、Preset 选择/JSON 导入、session 生命周期、流式聊天、Agent Run、非敏感恢复、50 条首屏窗口、加载更早、durable-ID DOM 复用与按消息回滚；对话主面板现可查看本轮实际 Persona/Preset/Provider/Model，以及有序装配材料、稳定性和估算规模。
...
- `PromptAssemblyTrace` 的 pipeline → 脱敏 HTTP preview → WebUI 用户摘要已闭环，但 revision 仍不完整：当前只有 Persona 数值 revision；角色卡、Preset、Worldbook、state/memory 尚未全部映射到统一不可变 revision，UI 会明确提示 unavailable，不得把文件时间冒充版本。
...
1. 在 #115 已交付的真实装配/HTTP/WebUI 可观察闭环上，补齐角色卡、Preset、Worldbook、state/memory 的统一 revision/provenance；不得用 mtime 伪造 revision；
2. 完成 #114 的 Persona 高级生命周期、Preset 管理与统一有效配置摘要；#126 的 v4 runtime、shared normalizer、主面板编辑和 PNG/JSON 端到端回归已交付，不得重复实现；
...
`main@13d07d7` 在 [push gate run 29488668196](https://github.com/GhostXia/AIRP/actions/runs/29488668196) 中通过全部代码门禁：

- `Rust workspace`：通过；
- `UI and WebUI`：通过；
- `Production topology`：通过。
- PR #194 的 [PR gate run 29488032908](https://github.com/GhostXia/AIRP/actions/runs/29488032908) 与 `CodeRabbit`：通过。

这些远端证据覆盖 workspace fmt/build/严格 Clippy/tests、warning-free rustdoc、神圣提示词不变式、Node 20.19 下的 UI typecheck 与 Vitest、WebUI 脚本/纯函数测试，以及真实 HTTPS/Chrome production topology；本次 topology 还覆盖 prompt preview 的生产静态资产与浏览器路径。该结果证明 `13d07d7` 与 PR #194 head；后续变更不得沿用为新结果。
