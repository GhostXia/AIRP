...
**P1 有限试用代码候选已形成；当前近期目标是继续开发可实际使用的版本，并把首聊黄金路径变成可重复验收的工程事实。**按以下顺序执行：

1. **继续开发 P1 黄金路径**：使用真实 provider、真实浏览器和生产拓扑贯通 onboarding → 首聊；优先补齐任何需要编辑磁盘 JSON、阅读开发文档或打开 dev tools 才能继续的产品缺口。
2. **建立可重复验收**：页面刷新后继续、服务重启后继续必须各自通过并独立记录；自动化覆盖、agent 辅助检查和维护者人工验收可以组合使用，同时记录候选版本、环境、失败阶段×类型和重试结果。
