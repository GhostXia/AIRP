...
- 当前已经形成 **P1 有限试用代码候选**，但仍是“待真实用户验证”，不是 P1 已通过，更不是正式发布：P1 要求少量自愿用户可完成首聊，并在页面刷新后和服务重启后分别恢复，常见失败可重试，且 secret 不泄露、关键资产不被静默损坏。完整 RP 资产生命周期、base lock / drift / rollback、版本化 migration、自动备份/恢复、可恢复删除、升级回滚、运维 runbook、浏览器矩阵与长会话 soak 仍属于 P2/P3 或正式发布门禁。
...
**P1 有限试用代码候选已形成；当前唯一近期目标是完成真实用户验证并修复由真实证据证明的阻塞。**按以下顺序执行：

1. **执行真实 P1 试用**：使用真实 provider、真实浏览器和生产拓扑，让符合 §2.2 画像的自愿用户完成 onboarding → 首聊；不得用 mock、agent、维护者自测或 CI smoke 代替市场证据。
2. **分别验证恢复路径**：页面刷新后继续、服务重启后继续必须各自通过并独立记录；至少 5 名符合画像的自愿用户中首聊完成率 ≥80%，不符合画像者单独报告、不计入分母，同时记录失败阶段×类型、重试结果与“是否愿意再次使用”诊断信息。
3. **只修真实阻塞**：任何首聊阻断、永久 loading、不可行动错误、secret 泄露、虚假成功或关键资产静默损坏立即修复并重新验证；未达到门槛时继续修黄金路径，不启动横向基础设施。
...
5. **P1 通过后再进入 P2/P3**：P2 选择一种高价值资产完成“工作副本/读取真值 → revision 完整性校验 → 差异比较 → 回滚 → 恢复 → 导出”的端到端 vertical slice；P3 再执行浏览器兼容、安全负向、旧数据升级、备份恢复、长会话 soak、artifact、签名与回滚演练。
