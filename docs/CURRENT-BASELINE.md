...
> 基线日期：2026-07-23（`main@03ffaf6`；PR #294 + PR #297 已合并）
>
> 已合并实现基线：`main@03ffaf6`；PR #294（WebUI 从权威样板重建）与 PR #297（暴露 delete/search/persona-unbind 操作）均已合并并通过全部 CI 门禁
>
...
> 2026-07-23 PR #297 合并后校准：WebUI 从权威样板重建（PR #294）、操作入口扩展（PR #297：delete/search/persona-unbind/state history）、Style Review/Soul-Drift/FTS5（PR #289–#293）和 adapter stream idle timeout 均已合并。项目最大风险仍是工程治理远超产品成熟度、首聊迟迟未有真实用户。以下顺序体现"先让产品活过来，再让它健康"的原则。
...
PR #297 squash-merge `03ffaf6` 在 [PR gate run 29938651425](https://github.com/GhostXia/AIRP/actions/runs/29938651425) 中通过全部代码门禁：

- `Rust workspace`：通过；
- `UI and WebUI`：通过；
- `Production topology`：通过。

PR #297 新增 `webui/tests/operations.test.mjs`（12 项测试），覆盖角色删除、会话删除、FTS5 搜索、Persona 删除/解绑、state history/schema 和 `<select>` helper 防御。

本基线吸收的最近合并序列（`88bef3e → 03ffaf6`）：PR #291（D 批次补全 + #290 遗留修复）→ PR #292（UI 权威样板）→ PR #293（#290 legacy FTS/Soul-Drift 修复）→ PR #294（WebUI 从样板重建 + 6-stage onboarding）→ PR #297（暴露 delete/search/persona-unbind 操作 + #295 helper 修复）。adapter stream idle timeout（`dcf14ed`）已在 PR #291 中合并，原 `STREAM-TIMEOUT-ISSUE.md` 已删除。
