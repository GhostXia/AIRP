...
> **战略筛选原则，不是全量移植授权**：所谓“好能力”必须通过当前 AIRP 用户需求、共享 domain service、最小授权和可验证验收四道筛选。源仓库存在不等于本仓应立即复制。当前优先级见 [PROJECT-AUDIT-2026-07-10.md](PROJECT-AUDIT-2026-07-10.md)。
...
| 好能力 | 来源（`D:\AIRP-Gateway`） | 融入哪 | 用途 |
|---|---|---|---|
| **MCP client + 传输**（stdio/streamable-HTTP + 连接池 + initialize 握手 + 协议版本协商） | `src/mcp/{client,pool,transport/*}.rs` | engine 的 **MCP-client 子系统** | §3.8 接**第三方** MCP 工具生态（agent 能力面=扩展面） |
...
- 两者不矛盾：内化自己的，开放接别人的。这正是"agent 能力面 = 扩展面"（§3.8）。

## 落地
- engine 已含 Core，直接演进。
- **M_AGENT-2** = MCP-Server 数据面内化主载体（[ABSORPTION](MCP-SERVER-ABSORPTION.md)）。
- Gateway 的 MCP-client + 安全硬化 + SSE 适配，随"第三方工具接入(§3.8)""web 就绪暴露端口(§0)""UI web 传输"三个需求点分别融入，不必一次全搬。
