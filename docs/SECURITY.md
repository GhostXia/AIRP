...
PR #218 delivered `tools/dep-governance/` as an offline supply-chain governance toolchain: dependency discovery across Cargo workspace and npm package-lock.json v3, audit routing (auto-pass / audit-required / block + five upgrade routes), and SPDX-2.3 / CycloneDX 1.5 SBOM plus human-readable third-party notices generated into `docs/sbom/`. #527/#554 wired an explicit `workflow_dispatch` exact-tag validation/publish code gate: `publish_release=true` checks the draft/prerelease tag, exact checkout/ref/`HEAD`, `--fail-on-block` and `--fail-on-unknown`, package/browser/desktop smoke, and all 17 audit-required records; #554 isolates the release-only `contents: write` context check while ordinary packaging remains read-only. There is no `release: published` pre-publish gate. Run `31309894372` on `main@affa315` completed 3 jobs successfully and uploaded 5 assets for `v0.0.5-rc.2`. Hosted environment configuration and a successful publish proof are not present as required-reviewer evidence: the hosted `release` environment API currently returns `protection_rules=[]` and `can_admins_bypass=true`, so no required reviewer is configured; the run proves candidate packaging only, not formal release assurance. The toolchain still does not replace per-dependency license/provenance verification at introduction time (see [DEV-GUIDE.md §7.1](DEV-GUIDE.md)).
