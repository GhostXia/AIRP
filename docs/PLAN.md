...
这不改变“两盒”“干净提示词”“数据单一真相”或客户端无关合同等既定原则。WebUI 当前是正式产品交付主面；Tauri/Vue 代码作为后续桌面客户端资产保留，但开发计划暂时搁置。[WEBUI-MVP-PLAN.md](WEBUI-MVP-PLAN.md) 只作为已完成的基础可用验收合同保留。
...
3. **近期收口顺序**：基础 WebUI、长历史和多 Persona 存储地基已由 PR #123/#124/#125/#127 完成；当前顺序为 production P0 → #114/#115/#126 RP 使用面 → 数据恢复/运维 → release candidate。#117/#87/#116 后移；桌面 #98/#29/#122 保留追踪但开发计划暂停。
