...
## 当前执行方向（2026-07-13：WebUI 正式上线）

PR #118、#119、#121、#123 已完成基础 WebUI RP 闭环；PR #124/#125 已完成 durable history 与窗口化，PR #127 已建立多 Persona 存储地基。当前开发目标正式调整为：**让 WebUI 能够作为单实例、自托管、单用户产品正式上线使用**。详细 release gates 与阶段顺序见 [WEBUI-PRODUCTION-PLAN.md](WEBUI-PRODUCTION-PLAN.md)。

1. **生产地基优先**：同源 HTTPS 入口、私有 engine、强制生产 access key、remote import policy、首方部署产物与 production smoke；
2. **闭合正式 RP 使用面**：按 #114/#115/#126 完成 Persona/Preset/Worldbook 管理、绑定、迁移报告、有效配置与 PromptAssemblyTrace 摘要；
3. **数据可靠性**：补版本化 migration、备份/恢复、可恢复删除、readiness、脱敏日志和运维 runbook；
4. **发布门禁**：覆盖真实浏览器、安全负向、旧数据升级、备份恢复、长会话 soak、artifact 与回滚演练；
...
这不改变“两盒”“干净提示词”“数据单一真相”或客户端无关合同等既定原则。WebUI 当前是正式产品交付主面；Tauri/Vue 保留为后续桌面客户端，但不阻塞 WebUI 首发。[WEBUI-MVP-PLAN.md](WEBUI-MVP-PLAN.md) 只作为已完成的基础可用验收合同保留。
...
- **UI 无关 + Web 正式交付（2026-07-13 更新）**：WebUI 是当前正式产品交付主面，Tauri/Vue 保留为后续桌面客户端。故**引擎必须是无头、独立的网络服务**（HTTP/SSE/WS，传输无关线协议），不嵌进任何单一 UI；所有客户端消费同一 engine 合同。
...
- **完成条件与执行路线**：[WEBUI-PRODUCTION-PLAN.md](WEBUI-PRODUCTION-PLAN.md) 是唯一正式上线合同；[WEBUI-MVP-PLAN.md](WEBUI-MVP-PLAN.md) 仅保留历史基础验收。
...
3. **近期收口顺序**：基础 WebUI、长历史和多 Persona 存储地基已由 PR #123/#124/#125/#127 完成；当前顺序为 production P0 → #114/#115/#126 RP 使用面 → 数据恢复/运维 → release candidate。#117/#87/#116 后移，桌面 #98/#29 不再阻塞 WebUI 首发。
