...
数据层开放题已进一步收敛：一个 `session_id` 定义一个独立开局/存档槽位；角色卡与全部启用世界书都物化为 session 内可编辑工作副本，第三方世界书先进入独立素材库；运行时以 session manifest 为唯一世界书加载清单，并用统一 `content_revision` 同时固定角色卡与世界书，保留每轮可复现性。当前仅 history/memory 已按命名 session 隔离，state、角色卡快照、第三方库、世界书物化与 revision 仍待分阶段实现。完整合同见 [SESSION-DATA-DESIGN.md](SESSION-DATA-DESIGN.md)（#168）。
