...
## 当前执行方向（2026-07-13：以 WebUI 孵化和稳定后端能力）

PR #118、#119、#121、#123 已完成基础 WebUI RP 闭环；PR #124/#125 已完成 durable message ID、cursor history、rollback-by-ID、50 条窗口、增量 DOM 与真实浏览器滚动保持。近期继续围绕 WebUI 增补开发，把它作为后端能力孵化、API/数据合同验证与基础 RP 使用的主开发面：

1. **完善 RP Profile**：当前处理 #114/#115，先建 shared PresetService、无损 sidecar、迁移报告/dry-run/revision，再补多 Persona、绑定与 PromptAssemblyTrace；
2. **长会话剩余项**：#37/#122 已完成 durable ID、cursor、WebUI window；继续保留 branch/swipe/edit、Tauri 虚拟滚动与 10k/100k 性能门；
...
3. **近期收口顺序**：基础 WebUI 与长历史 WebUI/backend 合同已由 PR #123/#124/#125 完成；当前顺序为 Persona/Preset 与 trace #114/#115 → ChangeInbox #117 → Agent-first 工作台 #87 → Style Review #116。#37/#122 保留剩余交互/性能项，桌面 #98/#29 是阶段性 release gate。
