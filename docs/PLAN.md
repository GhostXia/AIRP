...
- **状态**：⚠️ PR #141 已在 AIRP engine 交付 v2 `constant` 运行时合同、SillyTavern `constant` 归一化、WebUI 编辑与确定性 fixture；v1 数据兼容，多书 merge 不丢不同激活语义。shared normalizer、uid-keyed object/别名完整归一化、position/depth/selective/secondary/probability/matchWholeWords/递归和 import diagnostics 仍缺。后续继续拆成“统一解析/诊断”与“增量运行时语义”两阶段，不把 `constant` 重列为未交付。
...
6. **世界书插入引擎完整度**：关键词触发与 `constant` 已交付；下一步是优先 shared normalizer/import diagnostics，还是继续补 position/depth/selective/secondary/递归？且按 §3.2/TAVERN-PARITY §4——position/depth 这些机械插入语义要重组为“给 agent 的建议元数据 + 检索 Tool”，非硬编注入器。
