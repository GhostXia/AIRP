...
- 默认 Agent 工具注册表有 **15 个**工具：原 11 个加 state 读写与 lorebook 读写。执行需 engine-side `call:tool`、可选 allowlist，破坏性工具还需按名确认。
...
当前存在三种不同能力面：底层 Rust 模块、HTTP routes、15 个 Agent tools。文档不得把“底层已有”写成“Agent 已能调用”，也不得把 MCP-Server 的 38 个候选工具写成本仓已交付规格。后续扩展继续以共享 domain service 为前置。
...
**后续状态**：Chat/State/Lorebook services 已供 pipeline、HTTP 与 Agent tools 复用；registry 新增 state/lorebook 读写后为 15 个工具。自动生成 inventory 与 MCP adapter 仍后置。
