...
- Windows 便携 WebUI 只允许 `127.0.0.1` 同源运行；启动器必须把 `AIRP_DATA_DIR` 固定到包内 `data/`，把 `config.json` 固定到包根，并清除继承的 deployment/access/CORS 环境变量、显式禁用 `AIRP_ALLOW_LOCAL_PATH`；provider key 只写入包内 `data/secrets.json`，API/UI 不得回显，日志/诊断/支持包必须继续脱敏；
