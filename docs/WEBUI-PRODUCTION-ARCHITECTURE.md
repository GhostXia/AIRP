...
`done` event. The SSE exchange uses a synthetic session that is force-deleted after every attempt,
so the probe neither appends to a user conversation nor creates a pre-delete backup. A fixed
