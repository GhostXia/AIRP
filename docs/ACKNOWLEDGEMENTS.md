...
| [Nano ID](https://github.com/ai/nanoid) | `3.3.18` / 2026-09-07 | Vite → PostCSS 的传递依赖；#654 修复零长度自定义生成器安全告警，不新增直接依赖 | npm 官方 registry 元数据与已安装包 `LICENSE` 均为 MIT；锁定 [tarball](https://registry.npmjs.org/nanoid/-/nanoid-3.3.18.tgz)，SHA-512 integrity 见 lockfile 与同步生成的 SBOM；上游仓库 `ai/nanoid` | 采用 [GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8) 的 3.x 修复版，保留许可证及版权声明；当前 inventory 保守标为 runtime，不由工具链用途推断为不分发，也不声称已证明 AIRP 存在可利用路径 |
