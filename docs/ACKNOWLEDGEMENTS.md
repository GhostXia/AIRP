...
> 状态：**待持续更新的活文档**
>
> 最后仓库校准：2026-07-26，`main@200fed9`；本轮未重新查询全部上游版本，具体版本与许可证的实际核验日期仍以各表为准。`tools/dep-governance/` 提供 Cargo + npm 依赖发现与 SPDX/CycloneDX SBOM 生成器，当前 SBOM 快照存于 `docs/sbom/`；该工具是手动离线运行，不替代引入新依赖时的逐项许可证/provenance 核验。
