...
- **Status**: Partially mitigated for PR checks in PR #111; RR-008 release assurance remains open. `pr-gate.yml` runs Rust and UI quality gates without persisted checkout credentials. PR #218 upgraded GitHub Actions to `actions/checkout@v7` / `setup-node@v6` / `upload-artifact@v7` (the v6/v7 action wrappers themselves run on Node 24; workflow step `node-version` stays at 20.19.0 and is what UI/Vitest/WebUI tests execute under), and delivered `tools/dep-governance/` (dependency discovery + audit routing + SPDX-2.3 / CycloneDX 1.5 SBOM generation). The Windows workflow now has an explicit `workflow_dispatch` exact-tag validation/publish path and validates all 17 audit-required sign-offs, but no hosted `release` environment configuration or successful publish proof is present; do not mark RR-008 fully mitigated until that evidence exists.
...
- **Current control**: Required PR checks plus local/human review; checkout credentials are not persisted; explicit release dispatch validates the exact tag, package/smoke artifacts, `block`/unknown SBOM failures and all 17 audit-required sign-offs (identity, conclusion, obligations, reviewer evidence and expiry); the final job declares `environment: release`, rechecks draft/tag state, rejects duplicate assets and uploads a four-file SBOM allowlist without overwrite. The environment's required-reviewer policy and hosted evidence are not verified in this checkout.
