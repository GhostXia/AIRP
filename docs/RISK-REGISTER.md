...
- **Current control**: #564 is split into bounded PRs. The current WebUI remains the default and supported fallback; PR 2 delivers only the bounded Blueprint/Surface v2 protocol layer, and PR 3 adds a canonical-token desktop shell with a visibly labelled fixed fixture plus responsive/accessibility screenshot smoke. Browser preview does not construct MockBus. `/desktop/`, Surface API, v2 renderer and `HttpEngineBus` are not claimed as delivered. The target architecture keeps domain writes in existing Engine services, makes Surface projection reconstructable, forbids a Tauri business relay, and requires engine-authoritative Widget identity/grants. User assets are not migrated for UI restoration.
