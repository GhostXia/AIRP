...
- **Risk**: Packaged installer/runtime behavior, provider-backed remote smoke, automatic upstream version detection and hosted release-approval configuration remain outside routine PR CI; `audit-required` SBOM records still need human sign-off.
- **Current control**: Required PR checks plus local/human review; checkout credentials are not persisted; explicit release dispatch validates the exact tag, package/smoke artifacts, `block`/unknown SBOM failures and all 17 audit-required sign-offs (identity, conclusion, obligations, reviewer evidence and expiry); the final job declares `environment: release`, rechecks draft/tag state, rejects duplicate assets and uploads a four-file SBOM allowlist without overwrite. Run `31309894372` is the hosted candidate proof, but the environment API has no protection rules and allows admin bypass (`protection_rules=[]`, `can_admins_bypass=true`), so it does not prove required-reviewer approval; the environment's required-reviewer policy and hosted evidence are not verified for approval, while candidate hosted publish proof exists separately.
- **Required direction**: Configure and verify required reviewers on the hosted `release` environment, archive a successful validation/approval/publish proof, and only then mark the release-assurance portion mitigated. Keep release artifact smoke and SBOM validation as separate gates; add automatic upstream-version detection with de-duplicated issues (#192 follow-up slice); expand CI only when deterministic fixtures exist.
