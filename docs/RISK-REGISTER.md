...
> Last reviewed: 2026-08-24 for the #564 PR 7 candidate. RR-018 now includes the same-origin `/desktop/`, `HttpEngineBus`, and Widget host security-parity controls; runtime risks are not closed by documentation alone. Formal `v0.0.5` remains blocked by #130 real provider/browser/Compose acceptance and the missing required-reviewer policy on the `release` environment (`protection_rules=[]`, `can_admins_bypass=true`).
...
- **Status**: Partially mitigated in PR #111 through wire discriminant fixtures and engine-side capability/allowlist/confirm enforcement. PR #486/#491 (C-P3/C-P4) made widget capability authority engine-issued and engine-enforced per call (`/v1/widget-intents`, closed capability set, `GET /v1/grants` unified query). #564 PR 2 adds the Surface v2 machine authority plus shared Rust/TypeScript fixtures and parity tests; MCP/plugin grant subjects and generated bindings remain open.
- **Surface**: Rust and TypeScript protocol types are maintained manually; SSE event shapes are locked by `protocol/sse-events.json` (PR #464), widget intent shapes by `protocol/widget-intents.json`, and Surface v2 semantics by `protocol/surface-protocol-v2.json`; UI consent is cross-checked by engine authorization for widgets.
...
- **Current control**: Both sides have unit tests and runtime guards; Surface v2 has shared positive/negative/migration fixtures, bounded validation and stable error parity; widget capability enforcement is engine-authoritative per call with audit logging.
- **Required direction**: Generated bindings or equivalent schema enforcement for contracts still mirrored manually, and extension of the unified grant face beyond widgets.

## RR-008 · Automatic PR quality gate

- **Status**: Partially mitigated for PR checks and candidate packaging; RR-008 release assurance remains open. `pr-gate.yml` runs Rust and UI quality gates without persisted checkout credentials. PR #218 upgraded GitHub Actions to `actions/checkout@v7` / `setup-node@v6` / `upload-artifact@v7` (the v6/v7 action wrappers themselves run on Node 24; workflow step `node-version` stays at 20.19.0 and is what UI/Vitest/WebUI tests execute under), and delivered `tools/dep-governance/` (dependency discovery + audit routing + SPDX-2.3 / CycloneDX 1.5 SBOM generation). #527/#554 add an exact-tag `workflow_dispatch` path with least-privilege release-context isolation; the current workflow publishes only the Windows package after package/browser/desktop smoke. Dependency/audit snapshots remain committed under `docs/sbom/` for inspection from the tagged git tree, not as release assets or a CI sign-off gate. The hosted `release` environment API is `protection_rules=[]`, `can_admins_bypass=true`, so required-reviewer configuration is still missing; do not mark RR-008 fully mitigated until that evidence exists.
...
- **Risk**: Packaged installer/runtime behavior, provider-backed remote smoke, automatic upstream version detection and hosted release-approval configuration remain outside routine PR CI; dependency/audit snapshots are maintained in the tagged git tree but are not a release sign-off gate.
- **Current control**: Required PR checks plus local/human review; checkout credentials are not persisted; explicit release dispatch validates the exact tag and package/browser/desktop smoke; the final job declares `environment: release`, rechecks draft/tag state, rejects a duplicate `airp-webui-windows-x64.zip`, and uploads only that package without overwrite. Dependency governance remains available through the committed `docs/sbom/` snapshot and `tools/dep-governance/` scripts. The environment API has no protection rules and allows admin bypass (`protection_rules=[]`, `can_admins_bypass=true`), so it does not prove required-reviewer approval.
- **Required direction**: Configure and verify required reviewers on the hosted `release` environment, archive a successful validation/approval/publish proof, and only then mark the release-assurance portion mitigated. Keep package/browser/desktop smoke and dependency snapshot review explicit; add automatic upstream-version detection with de-duplicated issues (#192 follow-up slice); expand CI only when deterministic fixtures exist.
...
- **Status**: The 0.0.3 lockfile hardening slice remediates the three known npm advisories in this dependency graph; continue routine discovery under #192. This closes the reported findings, not the general risk of using a development toolchain.
- **Surface**: Locked Vite/Vitest/esbuild development and test dependencies under `ui/`.
- **Risk**: Development-server or test-UI exposure to an untrusted network can still turn a future dependency defect into a local development risk. Before this slice, the lockfile produced three advisories, including two high-severity nodes: [GHSA-3jxr-9vmj-r5cp](https://github.com/advisories/GHSA-3jxr-9vmj-r5cp), [GHSA-mh99-v99m-4gvg](https://github.com/advisories/GHSA-mh99-v99m-4gvg), and [GHSA-r28c-9q8g-f849](https://github.com/advisories/GHSA-r28c-9q8g-f849). These packages are not copied into the production gateway image, so the dependency-graph findings are not evidence of a production runtime compromise.
- **Current control**: Vite 8.1.4, Vitest 4.1.10 and `@vitejs/plugin-vue` 6.0.8 retain their bounded manifest ranges and exact npm lockfile. The lock-only transitive update records `brace-expansion` 2.1.1→2.1.4, `postcss` 8.5.16→8.5.25, and `nanoid` 3.3.15→3.3.16; direct package versions are unchanged. Node support starts at 20.19.0, development services remain loopback-only/trusted, and production serves independent static assets without `ui/node_modules`.
- **Current evidence**: Before/after comparison is three advisories and two high-severity nodes before the update versus `npm audit --json` total 0 and `npm audit --omit=dev --json` total 0 after it. The regenerated dependency inventory and SPDX/CycloneDX/notices outputs report zero unknown-license and zero blocked records; the new package records have registry integrity and MIT licensing. Existing UI typecheck, Vitest, WebUI, Tauri configuration, and real production HTTPS/Chrome topology gates remain required. A clean audit is evidence that these known advisories are absent from the current lock graph, not a claim that all future or unrelated vulnerabilities are impossible; `postcss` and related compiler/dev packages remain build-time reachability only, while production serves the built static assets.
...
- **Current control**: The launcher fixes `AIRP_DATA_DIR` and config inside the package, binds `127.0.0.1`, uses same-origin browser calls, clears inherited access/deployment/public-origin/CORS variables, and explicitly disables `AIRP_ALLOW_LOCAL_PATH`. Provider credentials use one versioned `data/secrets.json` that API/UI never echo; this is intentionally plaintext and protected only by the package directory's OS access boundary. Engine data paths retain traversal and atomic-write controls. Package smoke verifies the portable paths, security headers, local runtime mode, key restart, and real Chrome startup.
...
- **Status**: Partially mitigated. Runtime controls landed in PR #328; repository-owned ignore coverage on 2026-07-26; **DNS fail-closed + request-time re-resolve/pin** landed for v0.0.3 engine gate (#381 E-P0-3 / #329 N3). Plugin execution remains acceptable only for explicit trusted-user extensions — still **not** a multi-tenant code sandbox.
...
- **Current control**: #564 is split into bounded PRs. The current WebUI remains the default and supported fallback; PR 2 delivers the bounded Blueprint/Surface v2 protocol layer, PR 3 the canonical-token desktop shell, PR 4 the bounded client renderer/store/lifecycle, PR 5 the authenticated read-only Engine session Surface snapshot/SSE, PR 6 the same-origin `/desktop/` bundle plus browser/Tauri `HttpEngineBus`, and PR 7 Engine-authoritative Widget catalog/grants/plugins with WebUI/Vue opaque-frame parity. The Engine projection is reconstructable from domain services, has effective-root/character/session isolation, bounded replay, snapshot resync and closed-schema Activity failure receipts; it does not become RP truth or enter prompt assembly. Production `/desktop/` never selects MockBus or a Tauri business relay, Widget props remain the direct projection, malformed streams preserve last-known-good state, and package smoke covers both entries. Third-party ESM is digest-pinned, never imported into the Vue host process, receives only granted projection, and bridge traffic is bound to window/session/instance identity. Widget intent executors are still not delivered. User assets are not migrated for UI restoration.
