...
| 世界书 v4 解析+插入基础 | AIRP engine + C/M 参考 | 🔧 | `constant` + `selective`/`secondary_keys`、关键词触发、merge-safe 激活、presence-aware v3 迁移与 shared normalizer/导入诊断已交付；position/depth/probability/递归等高级语义仍需增量实现 |
