...
2. `post-pr-comment.mjs` 需在 Task 9 Step 2 创建。**B5 修复**：脚本必须先用 `fs.access` 检查 report 是否存在，不存在时优雅退出（exit 0 + 警告），不要让 `readFile` 抛 ENOENT 导致 step 失败——topology bootstrap 失败时 runner 不跑，report 不会生成，failure() placeholder step 已发占位评论。**B9 修复**：`AGENT_EXPLORATION_OPENAI_KEY` repo secret 未配置时（runner 启动后才发现 key 缺失，浪费 5+ 分钟 topology boot），workflow 必须在最早位置加 `Check LLM API key` 预检 step（输出 `configured=1/0`），所有后续 build/bootstrap/run step 用 `if: steps.check-llm-key.outputs.configured == '1'` 跳过；key 未配置时发友好评论（含配置路径），而不是让 runner 失败后由 failure() 占位评论兜底。`llm-client.mjs` 的 API key 缺失错误消息也应包含本地/CI 配置指引。
