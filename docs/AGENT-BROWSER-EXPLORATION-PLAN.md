...
注意：
1. `bootstrap-topology.sh` 必须在 Task 9 Step 1 实施前从 `deploy/production/smoke-ci.sh` 抽取并落地为独立脚本：仅启 mock provider + TLS + 临时数据拓扑、等待 health ready 后返回（不阻塞、不跑 smoke），并支持 `--teardown` 子命令做幂等清理。本计划不展开其代码（避免文档过长），但 workflow 中已直接调用该脚本，缺失会导致 Step 1 失败。**B4 修复**：脚本必须以 git 可执行权限（100755）提交，否则 workflow `./bootstrap-topology.sh` 会因 Permission denied (exit 126) 失败。命令：`git update-index --chmod=+x deploy/production/bootstrap-topology.sh`。**B6 修复**：脚本内部 `docker run --rm --entrypoint caddy airp-gateway:0.1.0 hash-password` 要求本地已 build `airp-gateway:0.1.0` 镜像（CI 干净环境没有该镜像，docker 会尝试 pull 触发 `pull access denied` exit 125）。workflow 必须在 Bootstrap step 之前加两个 step：(1) `cp .env.example .env` 生成临时 .env（compose.yaml 引用 `$AIRP_VERSION` 等变量，无 .env 时 build 失败）；(2) `docker compose --env-file .env -f compose.yaml build --pull`（与 pr-gate.yml 对齐）。bootstrap-topology.sh 内部会覆盖 .env 用于实际 topology，临时 .env 仅用于 build 阶段。**B7 修复**：`wait_for_engine_ready` 失败时必须 dump engine + gateway + mock_provider 日志到 stderr（与 smoke-ci.sh 对齐），让 CI annotations 直接看到根因而不用手挖 artifact；workflow 的 Upload artifacts step 必须把 `deploy/production/*.log` 也打包，作为 stderr 被截断时的备份诊断源。**B8 修复（方案 C 临时 umask）**：smoke-ci.sh 也有 `umask 077`，但 pr-gate.yml 在 smoke 前会先跑 `Create synthetic deployment inputs` step 预创建 secrets（默认 umask 022 = 0644），smoke-ci.sh 用 `>` 覆盖时 shell 保留原 0644 权限，engine uid 65532 能读。但 bootstrap-topology.sh 是独立调用，没有这个隐性前置，secrets 会是 0600，engine entrypoint 报 `missing required secret` 并无限重启。修复：TLS 私钥段保留 `umask 077`（与 smoke-ci.sh 对齐），secrets 段切回 `umask 022`，让 secrets 文件为 0644 可被 engine uid 65532 读取。不用全局 chmod（方案 A）也不删 umask 077（方案 B 会让 TLS 私钥变 0644）。
2. `post-pr-comment.mjs` 需在 Task 9 Step 2 创建。**B5 修复**：脚本必须先用 `fs.access` 检查 report 是否存在，不存在时优雅退出（exit 0 + 警告），不要让 `readFile` 抛 ENOENT 导致 step 失败——topology bootstrap 失败时 runner 不跑，report 不会生成，failure() placeholder step 已发占位评论。**B9 修复（方案 1 + 方案 3 组合）**：旧版方案是"`AGENT_EXPLORATION_OPENAI_KEY` secret 未配 → workflow 预检 skip 整 job"——这丢弃了 topology + harness + reporter + commenter 全链路可达性的验证机会。新方案：secret 未配 → `llm-client.mjs` 进入 **fallback 模式**（导出 `FALLBACK_MODE = !API_KEY`，不再 `process.exit(2)`），`runner.mjs` 在 `generateAndRunScript` 顶部检测 `FALLBACK_MODE`，跳过 `chatCompletion` 调用与重试循环，直接用 `getBuiltinSmokeScript()` 返回的内置 minimal smoke 脚本（只 navigate 4 个关键 screen + 调 `harness.getDomSnapshot()`，不做业务断言——失败意味着 topology/harness 自身有问题，不是 WebUI bug）。`getModel()` 在 fallback 模式下返回 `"gpt-4o-mini (fallback: built-in smoke script)"`，让 reporter 在 PR 评论中显示 fallback 标识，读者一眼能区分"本报告是管线自检，不是真 Agent 探索"。secret 已配时走真 LLM 路径（方案 1 正常流）。**不用 mock-provider.js 的原因**：它返回 RP 文本（"（轻轻点头）你说得对..."），`runner.mjs::extractCodeBlock()` 期望 ` ```js ` 代码块，二者不兼容；且 mock 已被 engine 用作 chat provider，复用会让"runner 是否真调到 LLM"模糊。workflow 不再做 LLM key 预检，所有 step 都跑——`OPENAI_API_KEY: ${{ secrets.AGENT_EXPLORATION_OPENAI_KEY }}` 在 secret 不存在时 GitHub 传空串，runner 自动进 fallback。`Publish failure placeholder comment` 的 `if` 简化为 `if: failure()`（不再依赖 `steps.check-llm-key.outputs.configured == '1'`）。删除 `Publish skipped (no LLM key) comment` step——fallback 模式下不再 skip，会产出真实报告。
