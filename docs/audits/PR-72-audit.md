...
## 5. v2 修订：审计发现修复跟踪（2026-07-06）

针对 v1 审计发现的 A2-A6，在本 PR 内追加修复 commit：

| 编号 | v1 严重度 | v2 状态 | 修复内容 |
|---|---|---|---|
| A1 | medium | **未修，留 issue** | W-06 历史消息无时间戳的 UX 不一致问题。涉及 engine 侧 `chat_store.jsonl` 格式扩展或 WebUI 读取 `ChatLogMeta` 显示会话时间范围，超出本 PR 范围，作为后续 issue 跟进 |
| A2 | low | **已修** | C3 改为按 PNG/JSON 路径动态计算 body 长度（`Math.ceil(file.size / 3) * 4 + 22` 或 `file.size + 16`），零误差拦截，替代原 7.5MB 固定阈值（[app.js:825-841](../../webui/app.js#L825-L841)） |
| A3 | low | **已修** | CSS 加 W-06 注释；`.ts` + `.role` 都显式设 `inline-block` + `vertical-align: baseline`，避免 display 属性混搭（[style.css:30-33](../../webui/style.css#L30-L33)） |
| A4 | low | **已修** | `replaceOptions` 为每个 `<option>` 设 `title = value`，截断时 hover 可看完整值（[app.js:209-221](../../webui/app.js#L209-L221)） |
| A5 | info | **已修** | `appendMsg` 注释"Date 或 null"改为"Date 或省略"，与实际调用方式一致（[app.js:444](../../webui/app.js#L444)） |
| A6 | info | **已修** | C3 注释"JSON 外壳约 30 字节"改为"22 字节（PNG 路径）/ 16 字节（JSON 路径）"，精度修正（[app.js:828-829](../../webui/app.js#L828-L829)） |
| A7 | info | **不修** | agent-input placeholder 示例引用 "loop-skeleton" 测试角色。WebUI 本身是 dev console，引用测试角色可接受 |

**v2 测试结果**：394 tests 全绿（339 lib + 19 集成 + 24 markdown + 12 security），2/2 神圣不变式通过。

**v2 终判**：A2-A6 已修复，A1 留 issue 后续跟进（涉及 engine 侧改动），A7 不修。**建议合并**。

---

*审计源: GLM-5.2 | 审计完成时间: 2026-07-06 | v2 修订时间: 2026-07-06 | 依据: AGENTS.md 审计守则三条*
