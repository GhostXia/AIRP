...
- 断言：
  ...
  - `done.stop_reason="converged"` 表明 agent loop 正常终止而非被截断；`steps_taken=3` 与请求的 `max_steps=3` 一致（不是被 max_steps 上限强行打断）；`tokens_estimated=316` 是 [agent/mod.rs:91-95](../../engine/src/agent/mod.rs#L91-L95) 的估算字段。
