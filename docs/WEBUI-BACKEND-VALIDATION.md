...
- 响应：HTTP 200 `content-type: text/event-stream`，真实 SSE 流（仅 `data:` 行，agent run 端点不发送 `event:` 前缀，参 [engine/src/agent/mod.rs:172](../../engine/src/agent/mod.rs#L172) `Event::default().data(...)`）：
...
- 断言：
  ...
  - **`max_steps=3` 与 plan 序列对应关系**（审计 H1）：源码 [agent/mod.rs:195-206](../../engine/src/agent/mod.rs#L195-L206) 显示 `max_steps >= 2` 时 plan = `[CallTool{echo}, Generate, Finish]`，正好 3 步。`steps_taken=3` 与请求的 `max_steps=3` 一致是因为 plan 序列长度 = 3，不是因为触及 step cap 上限。
  - 工具名是 `echo`（[agent/mod.rs:198](../../engine/src/agent/mod.rs#L198) 硬编码 `tool: "echo".to_string()`），params 是 `{"probe":"loop-skeleton"}`——M_AGENT-1 骨架阶段 plan 写死，未读取 user message 里的 tool 指令。模型输出里出现 `"I do not have access to a session_list tool"`，说明 user message 通过 subagent prompt 注入被模型看到，但 tool 选择不受其影响。
  - `delta.chunk` 字段是 `format!("{:?}", chunk)`（[agent/mod.rs:317](../../engine/src/agent/mod.rs#L317)）的 Rust Debug 字符串——`UnpackedChunk::Body("...")` 序列化为 `Body(\"...\")`。WebUI [app.js:481](../../webui/app.js#L481) 的 `summarizeAgentEvent` 直接 `chunk.chunk.slice(0, 60)` 显示这段 Debug 字符串（未做 un-Debug），是已知的 M_AGENT-1 UX 粗糙点，不影响事件序列完整性。
  - `done.stop_reason="converged"` 表明 agent loop 正常终止而非被截断；`steps_taken=3` 与请求的 `max_steps=3` 一致（不是被 max_steps 上限强行打断）；`tokens_estimated=316` 是 [agent/mod.rs:91-95](../../engine/src/agent/mod.rs#L91-L95) 的估算字段。**注意**（审计 H2）：`tokens_estimated` 是估算值而非精确值，由 [agent/mod.rs:313-314](../../engine/src/agent/mod.rs#L313-L314) 调用 `volume_store::estimate_tokens(&result.raw_acc)` 计算，精度取决于 `estimate_tokens` 的算法实现，不可作为账单或配额精确依据。
  - 持久化：**agent run 在 M_AGENT-1 骨架下不落库**（[agent/mod.rs:299-300](../../engine/src/agent/mod.rs#L299-L300) 注释 `run_generation_step 不 finalize`）。实测 `chat_log.jsonl` 仅追加 user 行（line 11），无对应 assistant 行。本项只验证 SSE 事件序列，不验证持久化。
...
- **覆盖范围声明（审计 F3）**：本项实测的是 connection refused（TCP RST）。`provider timeout`（upstream 接受连接但不响应）在 engine 侧走同一错误路径——[chat_pipeline.rs:803-826](../../engine/src/chat_pipeline.rs#L803-L826) `chunks_result_to_events` 的 `Err(String)` 分支统一把 reqwest 的 `error sending request`（含 transport error 与 timeout 两类）序列化为 `event: error`。**P0-4a 不实测 timeout**，但依据源码分支同源，timeout 行为可由本项外推。
...
- **P2 风险标注**：[adapter.rs:126-138](../../engine/src/adapter.rs#L126-L138) 的 chat 路径 `request.send().await` **没有 per-request timeout**（PR #38 给 `/v1/models` 配了 `MODELS_PROXY_TIMEOUT=5s`，但 chat 路径未享受同等待遇）。upstream 接受连接但不响应时 engine 会一直 hang 直到 OS 超时（可能数分钟）。这是 P2 可靠性层面的实际风险，本 PR 仅记录、不在 P0 范围内修复。

#### P0-4b：失败场景 — upstream 非 2xx 透传（实为 quota 429）

> **澄清（审计 F7）**：本项原标"未知 model"，但实际 upstream 响应是 `Cloud Error 429`（quota / rate limit），HTTP 500 是 Tracy 代理把上游 429 翻译后的结果。这并非"model not found"路径——`nonexistent-xyz-123` 因不在 provider 白名单内被 quota 拦截。但 engine 侧的透传行为对所有 non-2xx 一致（[chat_pipeline.rs:818-825](../../engine/src/chat_pipeline.rs#L818-L825) 把 upstream status + body 包成 `event: error`），所以证据对"upstream 非 2xx 透传"这一类失败模式仍然成立。**真正的"未知 model"（provider 返回 4xx 提到 model name）未被本项覆盖**，留 P0-4f。
...
P0 §9 清单状态（按 [WEBUI-BACKEND-PLAN.md §9](../../docs/WEBUI-BACKEND-PLAN.md#L242-L252) bullet 顺序，审计 F4 要求显式映射）：
