...
//!   防止并发追加在 `current.md` 中交错混合叙事内容。同步锁与文件操作
//!   在 `spawn_blocking` 中执行，避免占用 tokio worker。
...
impl Tool for NpcActionTool {
    ...
    fn call(
        ...
        Box::pin(async move {
            ...
            .await
            .map_err(|e| AirpError::Internal(format!("npc_action blocking task failed: {e}")))?
        })
    }
}
...
impl Tool for UpdateRelationshipTool {
    ...
    fn call(
        ...
        Box::pin(async move {
            ...
            // StateService::mutate 内部包含 std 锁与同步文件 I/O；整个
            // read-modify-write 在 blocking pool 执行，避免占用 tokio worker。
            let data_root = state.data_root.clone();
            tokio::task::spawn_blocking(move || -> Result<ToolResult, AirpError> {
                let snapshot = StateService::new(&data_root).mutate(&cid, |live| {
                    ...
                    let relationships = live_obj
                        ...
                        .ok_or_else(|| {
                            AirpError::Internal(
                                "relationships field is not a JSON object".to_string(),
                            )
                        })?;
            ...
            .await
            .map_err(|e| {
                AirpError::Internal(format!("update_relationship blocking task failed: {e}"))
            })?
        })
    }
}
