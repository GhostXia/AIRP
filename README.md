...
AIRP 不限制 Rust、Cargo、Node、npm、MSYS2 或缓存的安装盘符。请按各工具的标准方式安装，确保 `cargo`、`node` 和 `npm` 可从当前 shell 的 `PATH` 找到；自定义 `RUSTUP_HOME`、`CARGO_HOME`、npm prefix/cache 或 `CARGO_TARGET_DIR` 时，使用适合自己机器的路径。

维护者因本机 `C:` 盘空间不足，把整套工具链和缓存迁到了 `D:`。这只是该机器的本地覆盖，具体设置记录在 [AGENTS.md](AGENTS.md)，不是项目要求，也不应复制到其他贡献者环境。
